...
class Bird {
  ...
  public:
    ...
    void move(bool is_some_life) {
        time = clock.getElapsedTime().asMicroseconds();
        clock.restart();
        time = time / 2850;

        if (pos.y > window.getSize().y - image_hight && IsLife)
            IsLife = false;
        if (IsLife) {
            if (pos.y < image_up) {
                pos.y = image_up;
            ...
            if (pos.x + image_len + 10 > 0 && is_some_life) {
                pos.x -= 0.5 * time;
            }
        }
    }
...
class Net {
  ...
  public:
    ...
    void update(Int64 score) {
        for (int i = 0; i < 5; i++)
            x[i] += ((rand() % 30) - 15) / (score / 300);
    }

    bool net_res(double up_bar, double down_bar, double len_bar, double up,
                 double down) {
        bool result = false;
        double res = 0;
...
int main() {
    ...
    for (int i = 1; i < 5; i++) {
        barier[i].set_pos(barier[i - 1].get_x_pos() + 400, rand() % 550 + 100);
    }
    ...
    while (window.isOpen()) {
        ...
        while (IsSomeLife && window.isOpen()) {
            ...
            for (int i = 0; i < 4; i++) {
                if (int((score % power(10, i + 1)) / power(10, i)) == 0)
                    num[i].setTextureRect(IntRect(380, 0, 40, 60));
                else
                    num[i].setTextureRect(IntRect(
                        ((int((score % power(10, i + 1)) / power(10, i)) - 1) *
                         41.3) +
                            10,
                        0, 40, 60));
            }
            ...
            IsSomeLife = false;
            for (int i = 0; i < GENERATION_SIZE; i++) {
                if (bird[i].get_IsLife())
                    IsSomeLife = true;
                life_bird_1 = i;
            }
            for (int i = 0; i < life_bird_1; i++)
                if (bird[i].get_IsLife())
                    life_bird_2 = i;

            for (int j = 0; j < GENERATION_SIZE; j++) {
                if (net[j].net_res(
                        barier[current_bar].get_pos().y + 190 -
                            bird[j].get_pos().y,
                        bird[j].get_pos().y - barier[current_bar].get_pos().y,
                        barier[current_bar].get_pos().x - bird[j].get_pos().x,
                        window.getSize().y - bird[j].get_pos().y,
                        window.getSize().y - bird[j].get_d()))
                    bird[j].set_IsJump();

                bird[j].move(IsSomeLife);
                for (int i = 0; i < 5; i++) {
                    if (bird[j].get_pos().x > barier[i].get_pos().x &&
                        bird[j].get_pos().x <
                            barier[i].get_pos().x + bird[j].get_im_left())
                        if (bird[j].get_pos().y <
                                barier[i].get_pos().y + bird[j].get_im_down() ||
                            bird[j].get_pos().y > barier[i].get_pos().y + 190)
                            bird[j].die();
                }
            }

            for (int j = 0; j < GENERATION_SIZE; j++) {
                if (!bird[j].get_IsLife())
                    continue;
                bool IsBreak = false;
                for (int i = 0; i < 5; i++) {
                    if (bird[j].get_pos().x >
                            barier[i].get_pos().x + bird[j].get_im_left() + 8 &&
                        IsNewScore) {
                        score++;
                        current_bar++;
                        current_bar %= 5;
            ...
            for (int i = 0; i < 5; i++) {
                barier[i].move(IsSomeLife);
                if (barier[0].get_x_pos() + 100 < 0) {
                    barier[0].set_pos(barier[4].get_x_pos() + 400,
                                      rand() % 600 + 100);
                    IsNewScore = true;
                    break;
                }
                if (i != 0)
                    if (barier[i].get_x_pos() + 100 < 0) {
                        barier[i].set_pos(barier[i - 1].get_x_pos() + 400,
                                          rand() % 600 + 100);
                        IsNewScore = true;
                        break;
                    }
        ...
        for (int i = 0; i < GENERATION_SIZE; i++) {
            bird[i] = Bird();
            bird[i].set_image(rand() % 4 + 1);
            if (i != GENERATION_SIZE - 2)
                net[i].update(net_score);
        }
        score = 0;
        barier[0].set_pos(850, rand() % 550 + 100);
        for (int i = 1; i < 5; i++)
